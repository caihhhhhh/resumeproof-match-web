...
const content: Record<'zh' | 'en', Record<InfoKind, { eyebrow: string; title: string; intro: string; updated?: string; sections: Section[] }>> = {
  zh: {
    ...
    privacy: {
      eyebrow: '隐私政策', title: '你的简历，不应该悄悄留下来。', updated: '更新日期：2026 年 8 月 29 日',
      ...
      sections: [
        ...
        { title: '临时保存', paragraphs: ['草稿仅保存在当前浏览器标签页的临时存储中，用于刷新页面后的恢复。关闭标签页后，浏览器会清除这份临时草稿。本网站当前不建立账户，也不建立服务端个人简历库。'] },
        { title: '匿名访问分析', paragraphs: ['只有在你选择允许后，网站才会加载 Google Analytics 4，用于了解页面访问和关键流程是否顺利，例如开始分析、分析完成、导出和错误类别。我们不会向 GA4 发送简历或 JD 原文、文件名、职位链接、公司名称或职位名称。', '你的选择保存在当前浏览器的本地存储中，可以通过页尾的“分析设置”随时修改。托管与安全服务可能为防滥用或保持网站安全设置必要 Cookie。'] },
        { title: '第三方 AI 服务', paragraphs: ['开始匹配后，已确认的简历与 JD 文字会发送给 DeepSeek 完成语义分析。图片或扫描 PDF 仅在需要识别时发送给智谱视觉模型。第三方服务可能按照其自身条款和隐私规则处理请求，因此请避免提交与求职无关的敏感信息。'] },
  ...
  en: {
    ...
    privacy: {
      eyebrow: 'Privacy policy', title: 'Your resume should not quietly linger.', updated: 'Last updated: August 29, 2026',
      ...
      sections: [
        ...
        { title: 'Temporary storage', paragraphs: ['Drafts are stored temporarily in the current browser tab so they can survive a refresh. Closing the tab clears this temporary draft. The site currently has no accounts and does not maintain a server-side resume library.'] },
        { title: 'Anonymous usage analytics', paragraphs: ['Google Analytics 4 loads only after you allow it. It helps us understand page visits and whether key workflow steps succeed, such as starting analysis, completing analysis, exporting, and broad error categories. Resume or JD text, filenames, job links, company names, and role titles are never sent to GA4.', 'Your choice is stored locally in this browser and can be changed from “Analytics settings” in the footer. Hosting and security services may set essential cookies to prevent abuse and keep the site secure.'] },
        { title: 'Third-party AI services', paragraphs: ['After you start matching, confirmed resume and JD text is sent to DeepSeek for semantic analysis. Images or scanned PDFs are sent to a Zhipu vision model only when recognition is required. These providers may process requests under their own terms and privacy rules, so avoid unrelated sensitive information.'] },
