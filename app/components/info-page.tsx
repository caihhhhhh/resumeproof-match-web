  ...
  zh: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: '基础统计与可选分析', paragraphs: ['网站会记录不含简历正文、JD 正文、文件名、职位链接、公司名称或职位名称的基础访问与流程事件。若你选择“仅基础统计”，GA4 以 analytics_storage=denied 运行，不读写分析 Cookie，仅发送用于总体测量与建模的无 Cookie 信号；接口还会记录请求类型、成功或失败、耗时和宽泛错误类别。', '选择“允许完整分析”后，GA4 可以使用分析 Cookie，将同一次访问中的页面与步骤关联起来。广告存储、广告用户数据、广告个性化和 Google signals 始终关闭。你的选择保存在当前浏览器，可通过页尾“分析设置”随时修改。'] },
  ...
  en: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: 'Basic measurement and optional analytics', paragraphs: ['The site records basic visits and workflow events without resume text, JD text, filenames, job links, company names, or role titles. If you choose “Basic measurement only,” GA4 runs with analytics_storage=denied: it does not read or write analytics cookies and sends cookieless signals for aggregate measurement and modeling. The API also records request type, success or failure, duration, and broad error categories.', 'If you choose “Allow full analytics,” GA4 may use analytics cookies to connect pages and steps within a visit. Ad storage, ad user data, ad personalization, and Google signals remain disabled. Your choice is stored in this browser and can be changed from “Analytics settings” in the footer.'] },
